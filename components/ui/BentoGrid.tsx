    ...
    >
      <div className={`${id === 6} && 'flex justify-center h-full'`}>
        ...
        >
          {spareImg && (
            <img
              src={spareImg}
              alt={spareImg}
              className={"object-cover, object-center w-full -full"}
            />
          )}
        </div>
        {id === 6 && (
        ...
        >
          ...
          <div className="font-sans font-bold text-lg lg:tet-3xl max-w-96 z-10">
            {title}
          </div>
        </div>
          ...
          <div className="flex gap-1 lg:gap-5 w-fit absolute -right-3 lg:-right-2">
            <div className="flex flex-col gap-3 lg:gap-8">
              {["React.js", "Next.js", "TypeScript"].map((item) => (
                <span
                  key={item}
                  className="py-2 lg:py-4 lg:px-3 px-3 text-sx lg:text-base opacity-50 lg:opacity-100 rounded-lg text-center bg-[]"
                >
                  {item}
                </span>
              ))}
